...
FIELD_KEYS = {"id", "label", "name", "dataType", "validFilters"}
...
def normalize_snapshot(raw: Any) -> dict[str, Any]:
    ...
    for object_index, object_raw in enumerate(raw["objects"]):
        ...
        for field_index, field_raw in enumerate(object_raw["fields"]):
            ...
            field_id = _string(field_raw["id"], f"{field_path}.id")
            if field_id in seen_fields:
                raise SnapshotError(f"duplicate field id for {object_type}: {field_id}")
            seen_fields.add(field_id)

            name = field_raw["name"]
            if name is not None:
                name = _string(name, f"{field_path}.name")
    ...
    for item in snapshot["objects"]:
        lines.extend(
            [
                ...
                f"- Kind: `{item['kind']}`",
                "",
                "| Field | API/source name | Field ID | Data type | Valid filters |",
                "| --- | --- | --- | --- | --- |",
            ]
        )
...
def install(snapshot_path: Path, home: Path) -> dict[str, Any]:
    try:
    ...
    transactional_write(
        {
            ...
            claude_path: _managed_content(claude_existing, claude_block),
            codex_path: _managed_content(codex_existing, codex_block),
        }
    )
    ...
    result["files"] = {
        ...
        "claudeLoader": str(claude_path),
        "codexLoader": str(codex_path),
    }
    return result
...
def main() -> int:
    args = parse_args()
    try:
        result = install(args.snapshot_file.expanduser(), args.home.expanduser())
